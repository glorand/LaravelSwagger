...
gulp.task('copy', ['less'], function() {
  ...
  gulp
      .src(['custom/bootstrap/**/*'])
      .pipe(gulp.dest('../public/custom/bootstrap'))
      .on('error', log);

  // copy custom css folder
  gulp
      .src(['custom/css/**/*'])
...
gulp.task('clean_vendor_public', [], function() {
  return gulp
      .src('../../../public/vendor/laravel-swagger', {read: false})
      .pipe(clean({force: true}))
      .on('error', log);
});

gulp.task('copy_vendor_public', ['clean_vendor_public'], function() {
  return gulp
      .src(['../public/**/*'])
      .pipe(gulp.dest('../../../public/vendor/laravel-swagger'))
      .on('error', log);
});